...
def add_item():
    ...
    # 항목 데이터 초기화 및 저장
    predicted_days_left = current_stock / daily_usage
    predicted_empty_date = datetime.now() + timedelta(days=predicted_days_left)
    alert_date = predicted_empty_date - timedelta(days=alert_days_before)
    ...
    inventory[item_name] = {
        ...
        "predicted_empty_date": predicted_empty_date,  # 이 값이 여기서 설정됨
...
def feedback(item_name):
    ...
    # 피드백 데이터 기록: 날짜와 함께 소모량 변경 사항 저장
    new_feedback_row = {
        "date": feedback_date,
...
def predict(item_name):
    ...
    predicted_usage = [item["daily_usage"]] * len(dates)  # 일정한 소비량 가정
    
    plt.plot(dates, predicted_usage, label="Predicted Usage")
    plt.axvline(item["predicted_empty_date"], color='red', linestyle='--', label=f"Out of Stock: {item['predicted_empty_date'].date()}")
    plt.axvline(item["alert_date"], color='blue', linestyle='--', label=f"Alert Date: {item['alert_date'].date()}")
    plt.xlabel('Date')
